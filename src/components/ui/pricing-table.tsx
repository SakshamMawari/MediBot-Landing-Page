...
function PricingTableCell({
	...
	return (
		<td
			...
			className={cn('p-4 align-middle whitespace-nowrap text-black dark:text-white', className)}
			{...props}
		>
			{children === true ? (
				<CheckIcon aria-hidden="true" className="size-4 text-black dark:text-white" />
			) : children === false ? (
				<MinusIcon
					aria-hidden="true"
					className="text-black dark:text-white size-4"
				/>
			) : (
				children
			)}
...
function PricingTableHead({ className, ...props }: React.ComponentProps<'th'>) {
	return (
		<th
			...
			className={cn(
				'p-2 text-left align-middle font-medium whitespace-nowrap text-black dark:text-white',
				className,
			)}
			{...props}
		/>
		...
		>
			<div className="flex items-center gap-2">
				<div className="flex items-center justify-center rounded-full border p-1.5">
					{Icon && <Icon className="h-3 w-3 text-black dark:text-white" />}
				</div>
				<h3 className="text-black dark:text-white font-mono text-sm">{name}</h3>
				<Badge
					variant="secondary"
			...
			<div className="mt-4 flex items-baseline gap-2">
				...
				{compareAt && (
					<span className="text-black dark:text-white text-sm line-through opacity-60">
						{compareAt}
					</span>
				)}
			</div>
